...
class PTQSLQuantMatMul(MinMaxQuantMatMul):
    ...
    def __init__(self, A_bit=8, B_bit=8, mode="raw", metric="mse", search_round=1, eq_n=100, 
                 head_channel_wise=True, num_heads=12):
...
class PTQSLBatchingQuantMatMul(PTQSLQuantMatMul):
    def __init__(self, A_bit=8, B_bit=8, mode="raw", metric="mse", calib_batch_size=32, 
                 search_round=1, eq_n=100, head_channel_wise=True, num_heads=12):
        super().__init__(A_bit, B_bit, mode, metric, search_round, eq_n, head_channel_wise, num_heads)
...
class AsymmetricallyBatchingQuantMatMul(PTQSLBatchingQuantMatMul):
    def __init__(self, A_bit=8, B_bit=8, mode="raw", metric="mse", calib_batch_size=32, 
        ...
        super().__init__(A_bit, B_bit, mode, metric, calib_batch_size, search_round, 
                         eq_n, head_channel_wise, num_heads)
    ...
    def calculate_percentile_candidates(self, x, quantizer, l=0.9, r=1.0):
        num_zp = min(16, quantizer.n_levels * 2)
        ...
        while tensor_too_large:
            try:
                a_uppers_candidates = torch.quantile(x.view(mini_batch_size, -1), pct.to(x.device), dim=-1).mean(dim=-1) # shape: 2
                a_lowers_candidates = torch.quantile(x.view(mini_batch_size, -1), (1-pct).to(x.device), dim=-1).mean(dim=-1) # shape: 2
                tensor_too_large = False
            except:
                mini_batch_size *= 2
        delta_min = (a_uppers_candidates[0:1] - a_lowers_candidates[0:1]).view(1, 1, 1, 1, 1)
        delta_max = (a_uppers_candidates[1:] - a_lowers_candidates[1:]).view(1, 1, 1, 1, 1)

        splits = torch.linspace(0, 1, steps=num_scale).cuda()[:, None, None, None, None] * (delta_max - delta_min)
        a_scale_candidates = ((delta_min + splits).repeat(num_zp, 1, 1, 1, 1) / (2 * quantizer.n_levels - 1)).clamp(min=1e-5)
        a_scale_candidates = a_scale_candidates.repeat(1, *quantizer.scale.shape)
        a_scale_candidates = torch.cat([a_scale_candidates, a_scale_candidates[-1:]], dim=0)
        zp_min = int(quantizer.n_levels - num_zp / 2)
        zp_max = int(quantizer.n_levels + num_zp / 2)
        ...
        a_zero_point_candidates = a_zero_point_candidates.repeat(1, *quantizer.scale.shape)
        a_zero_point_candidates = torch.cat([a_zero_point_candidates, a_zero_point_candidates[-1:]], dim=0)
        return a_scale_candidates, a_zero_point_candidates
        
    def hyperparameter_searching(self):
        self._initialize_calib_parameters()
        A_scale_candidates, A_zero_point_candidates = self.calculate_percentile_candidates(self.raw_input[0].cuda(), self.A_quantizer)
        B_scale_candidates, B_zero_point_candidates = self.calculate_percentile_candidates(self.raw_input[1].cuda(), self.B_quantizer)
...
class PostSoftmaxAsymmetricallyBatchingQuantMatMul(AsymmetricallyBatchingQuantMatMul):
    def __init__(self, A_bit=8, B_bit=8, mode="raw", metric="mse", calib_batch_size=32, search_round=1, 
                 eq_n=100, head_channel_wise=True, num_heads=12):
        super().__init__(A_bit, B_bit, mode, metric, calib_batch_size, search_round, 
                         eq_n, head_channel_wise, num_heads)
        del self.A_quantizer
        target_shape = [1, self.num_heads, 1, 1] if head_channel_wise else [1, 1, 1, 1]
